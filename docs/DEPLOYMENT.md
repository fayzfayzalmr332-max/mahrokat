...
|---|---|
| أي Push أو Pull Request | ① pytest كامل (حارس المنطق المالي) ② تطبيق المخطط على **سيناريوهين**: صفحة بيضاء + **محاكاة قاعدة قديمة (001+002)** — لكلٍّ إعادة تطبيق ثانية (إثبات Idempotency) ومطابقة الكائنات (6/6/5/4) |
...
## `42703 — column ... does not exist` (في خطوة db-deploy)
كان سببه ترتيب داخل ملف التوحيد: توثيق أعمدة تُضاف لاحقاً (`updated_at` / `seq` / `external_ref`)
بينما قاعدة الإنتاج تحوي جداول 001+002 القديمة التي لا تعرف هذه الأعمدة بعد. **عولج جذرياً**:
نُقل التوثيق إلى §2.4 (بعد تمريرة التوافق التي تضمن وجود الأعمدة)، وأُضيف في CI سيناريو
**«محاكاة قاعدة قديمة»** يطبّق 001+002 الأصليين ثم المخطط الموحد فوقهما — فلا يتكرر هذا الصنف أبداً.
إن ظهر 42703 مجدداً لعمود آخر: أرسل سطري الخطأ وسأعالجه فوراً.
